...
frappe.views.Workspace = class Workspace {
	...
	async show_page(page) {
		...
		if (this.workspaces.length) {
			...
			this.render_erpz_workspace_announcements();
...
render_erpz_workspace_announcements() {
	try {
		console.log("[ERPZ ANN] render called", {
			page_name: this._page?.name,
			page_title: this._page?.title,
		});
		frappe.show_alert({ message: "ERPZ ANN: metodo executado", indicator: "green" }, 5);

		this.body.find("#erpz-workspace-announcements").remove();

		const probe = `
			<div id="erpz-workspace-announcements"
				style="
					position:fixed;
					top:90px;
					left:50%;
					...
					border-radius:10px;
				">
				ERPZ ANN TESTE FIXO - se voce esta vendo isso, o JS custom carregou.
			</div>
		`;

		this.body.append(probe);
	} catch (e) {
		console.error("[ERPZ ANN] render error", e);
	}
}

mount_erpz_workspace_rotator(prefix, cards, intervalMs) {
	let idx = 0;
	const icon = document.getElementById(prefix + "-icon");
	const title = document.getElementById(prefix + "-title");
	const text = document.getElementById(prefix + "-text");
	const cta = document.getElementById(prefix + "-cta");
	const link = document.getElementById(prefix + "-link");
	const dotsWrap = document.getElementById(prefix + "-dots");
	if (!icon || !title || !text || !cta || !link || !dotsWrap) return;

	dotsWrap.innerHTML = "";
	const dots = cards.map((_, i) => {
		const d = document.createElement("button");
		d.type = "button";
		d.className = "erpz-ann-dot" + (i === 0 ? " active" : "");
	...
	const render = () => {
		...
		link.href = card.href || "#";
		link.style.display = card.cta && card.href ? "block" : "none";
	...
	this._erpz_ann_intervals = this._erpz_ann_intervals || [];
	this._erpz_ann_intervals.push(timerId);
}

inject_erpz_workspace_announcements_style() {
	if (document.getElementById("erpz-workspace-ann-style")) return;

	const style = document.createElement("style");
	style.id = "erpz-workspace-ann-style";
	style.textContent = `
		#erpz-workspace-announcements{
			max-width:820px;
			margin:24px auto 0;
			display:grid;
		...
		.erpz-ann-header{display:flex;align-items:center;gap:8px;}
		.erpz-ann-icon{
			width:18px;height:18px;border-radius:50%;
			display:inline-flex;align-items:center;justify-content:center;
			font-size:11px;border:1px solid #d1d5db;color:#6b7280;
		}
		.erpz-ann-title{font-weight:700;font-size:14px;color:#111827;}
		.erpz-ann-text{margin-top:6px;font-size:13px;line-height:1.45;color:#4b5563;}
		.erpz-ann-btn{margin-top:10px;}
		.erpz-ann-dots{margin-top:8px;display:flex;justify-content:center;gap:6px;}
		.erpz-ann-dot{
			width:6px;height:6px;border-radius:9999px;border:0;cursor:pointer;background:#d1d5db;
		}
		.erpz-ann-dot.active{width:14px;background:#6b7280;}
	`;
